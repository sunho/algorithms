...
struct bigint {
    ...
    bool operator<(const bigint &v) const {
        ...
        if (a.size() == 1 && v.a.size() == 1) {
          return sign == 1 ? a[0] < v.a[0] : a[0] > v.a[0];
        }
...
const bigint def = -(ll)1e15;
template<class T, auto combine>
struct seg_tree {
  int n;
  vector<T> bits;
...
bigint combine_max(const bigint& a, const bigint& b) { return max(a,b); }
using max_seg_tree = seg_tree<bigint,combine_max>;
 
bigint combine_min(const bigint& a, const bigint& b) { return min(a,b); }
using min_seg_tree = seg_tree<bigint,combine_min>;
...
void solve() {
  ...
  bigint sum = accumulate(begin(A),end(A),bigint(0));
  vector<bigint> pf(n+1);
  for (int i=0;i<n;i++){
    pf[i+1] = pf[i] + A[i];
  }
  vector<bigint> dp(n+2);
  max_seg_tree dp2(n+2);
  bigint ng = 0, ok = sum+1;
  auto check = [&](bigint cost) {
    for (int i=0;i<=n+1;i++){
      dp[i] = 0;
    }
    for (int i=0;i<=n+1;i++){
    ...
    dp2.set(0,0);
    for (int i=1;i<=n+1;i++){
      int k = (int)(ranges::lower_bound(pf,pf[i-1]-cost)-begin(pf));
      if (k <= i-1) {
