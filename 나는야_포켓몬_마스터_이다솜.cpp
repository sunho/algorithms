...
int rand_int(int n) {
  static mt19937 rng((mt19937::result_type)chrono::steady_clock::now().time_since_epoch().count());
...
struct poly_hash {
  ...
  static int base;
  vector<hint> ha, pw;
  poly_hash(string& str) : ha((int)str.size()+1), pw(ha) {
    pw[0] = 1;
...
void solve() {
  ...
  for(int i=0;i<n;i++){
    cin >> A[i];
    inv[poly_hash::hash_string(A[i]).val()] = i;
  }
  while(q--){
    string t;
    cin >> t;
    if (!t.empty() && all_of(all(t), ::isdigit)) {
      int i = stoi(t);
      i--;
      cout << A[i] << "\n";
    } else {
      cout << inv[poly_hash::hash_string(t).val()] + 1 << "\n";
    }
  }
}
